...
public class MainCongifurationAndSelectionBasedSummaries {

	public static final String DB = "jdbc:sqlite:database/codeFragments.sqlite";
		
	public static void main(String[] args) throws Exception {
		Database.getInstance().initConnection(MainCongifurationAndSelectionBasedSummaries.DB);
	...
	public static void generateSummaries(Connection c, Source source) 
		...
		List<Integer> cids = DatabaseFeatures.getCidsSorted(c, source.getDbSourceString());
	...
	public static String  generateSummary(Connection c, int cid) 
			 throws SQLException, IOException {
		return generateSummary(c, cid, IncludeOrNot.IncludeProbably);
	}
	
	public static String  generateSummary(Connection c, int cid, IncludeOrNot threshold) 
			 throws SQLException, IOException {
		
		List<Integer> uids = DatabaseSignatureSelection.getAllUids(c, cid);		
		List<Integer> uidToExclude = deselectMethodAndTypeSignatures(c, cid, threshold);
	...
	public static List<Integer> deselectMethodAndTypeSignatures(Connection conn, int cid, 
			IncludeOrNot threshold)
			throws SQLException, IOException {
		...
		for( Entry<Integer,MethodSignature>  e : methodSignatureUids.entrySet()) {
			...
			if( method.getMethodConfiguration().ordinal() <= threshold.ordinal()) {
	...
	public static List<Integer> deselectLines(Connection conn, int cid) throws SQLException, IOException {
